"""Combine weather and MRT parquet shards into combined hourly parquet outputs."""
...
def _discover_weather_city_shards(weather_root: str) -> dict[int, list[str]]:
    ...
    try:
        ...
        LOGGER.warning(
            "Could not list weather parquet files under %s: %s",
            weather_root,
            exc,
        )
        return {}
...
def _load_city_lookup(weather_shard_count: int) -> DataFrame:
    ...
    shard_size = max(
        1,
        (len(city_lookup) + weather_shard_count - 1) // weather_shard_count,
    )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--weather-city-shard-count", type=int, default=None)
...
def _process_standard_weather_shards(
    ...
    if not selected_shards:
        msg = "No matching weather and MRT shards found for the requested filters."
        raise RuntimeError(msg)
...
def _process_city_sharded_weather(args: argparse.Namespace) -> None:
    ...
    if not weather_city_shards:
        msg = f"No weather parquet shards found under {args.weather_root}."
        raise RuntimeError(msg)

    selected_shards = _select_requested_shards(mrt_shards.keys(), args)
    if not selected_shards:
        msg = "No MRT shards found for the requested filters."
        raise RuntimeError(msg)

    weather_shard_count = args.weather_city_shard_count
    if weather_shard_count is None:
        weather_shard_count = max(weather_city_shards) + 1

    city_lookup = _load_city_lookup(weather_shard_count)
    for shard_key in selected_shards:
