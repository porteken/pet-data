...
3. `pull_weather.py` and `pull_mrt.py` split the CDS ingestion by dataset, with shared CDS/retry/filesystem helpers in `pull_cds_shared.py`. Weather uses point-based ERA5 single-levels timeseries pulls, can scope to the full historical window or an explicit `year/month`, and writes parquet partitions under `city_shard_index=<n>/`. MRT uses tiled UTCI pulls over that same moving window and writes parquet partitions under `year=<year>/tile_id=<tile_id>/` for full-year pulls or `year=<year>/month=<mm>/tile_id=<tile_id>/` for month-scoped pulls.
