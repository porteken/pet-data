...
Wet-bulb temperature data (NLDAS-2/Davies-Jones) has moved to the sibling [`wetbulb-data`](../wetbulb-data) repo, which shares this project's Postgres instance and S3 bucket.

## Entry points

- `pipeline.py` orchestrates the full pipeline locally (fetch → compute → load → views). `pull_all.sh` and `pull_pet.sh` are thin wrappers around it, and the historical environment variables (`MODE`, `YEARS`, `PRODUCTS`, `USE_CLOUD_RUN`, ...) are still honored as defaults. Run `pipeline.py --help` for the full flag list; `--mode smoke` processes a single week for quick validation.
