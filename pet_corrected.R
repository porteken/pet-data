...
PETcorrected <- function(
    ...
    systemp <- function(
        ...
        if (is.null(tair)) {
            if (verbose) {
                warning(
                    "Air temperature not set in function, setting to 21 degrees C"
                )
            }
            tair <- 21
        }
        if (is.null(tmrt)) {
            if (verbose) {
                warning(
                    "Mean radiant temperature not set in function, setting to 21 deg. C"
                )
            }
            tmrt <- 21
        }
        if (is.null(v_air)) {
            if (verbose) {
                warning("Air velocity not set in function, setting to 0 m/s")
            }
            v_air <- 0
        }
        if (is.null(rh)) {
            if (verbose) {
                warning("Relative humidity not set in function, setting to 50%")
            }
            rh <- 50
        }
        if (is.null(M)) {
        ...
        if (is.null(Icl)) {
            if (verbose) {
                warning("clothing level not set, setting to 0.9.")
            }
            Icl <- 0.9
        }
        ...
        if (length(unique(lengths)) > 2) {
            stop(
                "Length of inputs differ! Either a single value or a vector of values of the same length should be used"
            )
        ...
        facl <-
            (173.51 * icl - 2.36 - 100.76 * icl * icl + 19.28 * `^`(icl, 3.0)) /
            100.0
        ...
        met_base[sex == "female"] <-
            ...
            (1.0 +
                0.004 * (30.0 - age) +
                0.018 * (ht * 100.0 / `^`(mbody, 1.0 / 3.0) - 42.1))
            ...
            while (T) {
                for (count2 in 1:100) {
                    ...
                    if (any(c[, 11] >= 0.0)) {
                        ...
                        tcore[ww, 6] <-
                            (-c[ww, 10] - `^`(c[ww, 11], 0.5)) /
                            (2.0 * c[ww, 5]) # Numerical safety to avoid negative roots
                        tcore[ww, 1] <-
                            (-c[ww, 10] + `^`(c[ww, 11], 0.5)) /
                            (2.0 * c[ww, 5])
                    }
                    ...
                    if (any(c[, 9] >= 0.0)) {
                        ww <- which(c[, 9] >= 0.0)
                        tcore[ww, 2] <- (-c[ww, 7] + `^`(abs(c[ww, 9]), 0.5)) /
                            (2.0 * c[ww, 5])
                        tcore[ww, 5] <- (-c[ww, 7] - `^`(abs(c[ww, 9]), 0.5)) /
                            (2.0 * c[ww, 5])
                    }
                    ...
                    swm[ww] <- 0.0
                    swm[which(sex == "female")] <- swm[which(sex == "female")] *
                        0.7
                ...
                if (count1 == 3.0 && (j != 2 && j != 5)) {
                    if (j != 6 && j != 1) {
                        if (j != 3) {
                            ...
                            if (
                                any(tcore[, j] >= tc_set) || any(tsk > tsk_set)
                            ) {
                                g100 <- FALSE
                                break
                            }
                        ...
                        if (
                            any(c[, 11] < 0.0) ||
                                (any(tcore[, j] >= tc_set) || any(tsk <= 33.85))
                        ) {
                            g100 <- FALSE
                            break
                        }
    ...
    pet <- function(tc, tsk, tcl, ta_init, esw_real) {
        ...
        met_base <- 3.19 *
            ...
            (1.0 +
                0.004 * (30.0 - age) +
                0.018 * (ht * 100.0 / `^`(mbody, 1.0 / 3.0) - 42.1))
        met_base[which(sex == "male")] <- met_base <- 3.45 *
            `^`(mbody, 0.75) *
            (1.0 +
                0.004 * (30.0 - age) +
                0.01 * (ht * 100.0 / `^`(mbody, 1.0 / 3.0) - 43.4))
        ...
        while (count1 != 400) {
            ...
            tx[enbal <= 0.0] <- tx[enbal <= 0.0] + xx[enbal <= 0.0]
            bitmask2 <- (enbal > 0.0 & enbal2 >= 0.0) |
                (enbal < 0.0 & enbal2 <= 0.0)
